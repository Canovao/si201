...
int insere_ord_arvbin  (Nodo_AB **AB, int Dado);
int remove_dado_arvbin (Nodo_AB **AB, int Dado);
int exibe_ab_infixado  (Nodo_AB *AB);
int exibe_ab_prefixado (Nodo_AB *AB);
int exibe_ab_posfixado (Nodo_AB *AB);
int maior_arvbin       (Nodo_AB *AB, int *Dado_out);
...
int apaga_arvbin (Nodo_AB **AB); //-- apaga toda a arvore - FEITO
int conta_nodos_arvbin (Nodo_AB *AB); //-- conta quant. de nós na arvore - FEITO
int pesquisa_arvbin (Nodo_AB *AB, int Dado, int *resp); //-- procura por dado na árvore - FEITO
int menor_arvbin (Nodo_AB *AB, int Dado, int *menor); //-- obtem o primeira menor que - FEITO
int maior_arvbin (Nodo_AB *AB, int Dado, int *maior); //-- obtem o primeira maior que - FEITO
int sucessor_arvbin (Nodo_AB *AB, int Dado_in, int *Dado_out); //-- obtem filho direito de - FEITO
int predecessor_arvbin (Nodo_AB *AB, int Dado_in, int *Dado_out); //-- obtem pai de - FEITO

int verifica_vazio(Nodo_AB *AB){
    return (AB != NULL) ? 0:1
}

int predecessor_arvbin(Nodo_AB *AB, int Dado_in, int *Dado_out) {
    if (verifica_vazio(AB) == 0) {
        if (Dado_in == AB->Dado) {
...
int sucessor_arvbin (Nodo_AB *AB, int Dado_in, int *Dado_out) {
    if (verifica_vazio(AB) == 0) {
        if (Dado_in == AB->Dado) {
...
int maior_arvbin(Nodo_AB *AB, int Dado, int *maior) {
    if (verifica_vazio(AB) == 0){
        if (AB->Dado > Dado) {
            *maior = AB->Dado;
            return 0;
        } else {
            return menor_arvbin(AB->FilhoEsq);
        }
        return 0;
    }
    return 1;
...
int menor_arvbin(Nodo_AB *AB, int Dado, int *menor) {
    if (verifica_vazio(AB) == 0){
        if (AB->Dado < Dado) {
            *menor = AB->Dado;
            return 0;
        } else {
            return menor_arvbin(AB->FilhoDir);
        }
        return 0;
    }
    return 1;
...
int conta_nodos_arvbin(Nodo_AB *AB){
    int qtd = 0;
    // se AB não vazio conta +1
    if (verifica_vazio(AB) == 0){
        qtd++;
        // se AB tem nó à esquerda roda a análise nesse nó
        if(verifica_vazio(AB->FilhoEsq) == 0){
            qtd += conta_nodos_arvbin(AB->FilhoEsq);
        }
        // se AB tem nó à direita roda a análise nesse nó
        if(verifica_vazio(AB->FilhoDir) == 0){
            qtd += conta_nodos_arvbin(AB->FilhoDir);
        }
    }
    return qtd;
}
...
int pesquisa_arvbin(Nodo_AB *AB, int Dado, int *resp) {
    if (verifica_vazio(AB) == 0) {
        if (Dado == AB.Dado){
            *resp = 0;
            return 0;
        } else if (Dado > AB.Dado) {
            return pesquisa_arvbin(AB->FilhoDir, Dado, resp);
        } else if (Dado < AB.Dado) {
            return pesquisa_arvbin(AB->FilhoEsq, Dado, resp);
        }
    }
    return 1;
}